...
SYS_BULL = (
    ...
    "同时诚实面对反方证据，不许凭空捏造。"
    "【关键要求】每条理由必须写出具体算式或数据对比，并注明数据的时间区间。"
    "例如：'近20日涨幅+15%，跑赢沪深300指数+8%，超额收益7个百分点（数据期：2025-01-01至2025-01-20）'。"
    "禁止只说'技术面看好'这种空话。" + RULES
)
...
SYS_BEAR = (
    ...
    "同时诚实面对反方证据，不许凭空捏造。"
    "【关键要求】每条理由必须写出具体算式或数据对比，并注明数据的时间区间。"
    "例如：'近60日回撤-22%，超过沪深300同期-10%的跌幅，相对弱势12个百分点（数据期：2024-11-01至2025-01-10）'。"
    "禁止只说'风险较大'这种空话。" + RULES
)
